...
export class EventService {
    ...
    async getEvents(): Promise<SupabaseEvent[]> {
        const response = await this.supabaseService.request(
            'events?select=id,name,event_date&order=event_date.desc'
        );
    ...
    async getEventById(id: number): Promise<SupabaseEvent> {
        const response = await this.supabaseService.request(
            `events?select=id,name,event_date&id=eq.${id}&limit=1`
        );
    ...
    async createEvent(name: string, eventDate: string): Promise<SupabaseEvent> {
        const response = await this.supabaseService.request('events', {
            method: 'POST',
            headers: { Prefer: 'return=representation' },
            body: { name, event_date: eventDate }
        });
    ...
    async updateEvent(id: number, name: string, eventDate: string): Promise<void> {
