...
import { IndividualStatsService } from '../../services/individual-stats.service';

type SortColumn = keyof Omit<SupabasePlayerStatsRow, 'player_id'> | 'name';
type SortDirection = 'asc' | 'desc';

@Component({
    selector: 'ch-individual-stats',
...
export class IndividualStatsComponent implements OnInit {
    ...
    async ngOnInit(): Promise<void> {
        try {
            ...
            if (event) {
                this.rows = await this.individualStatsService.getIndividualStatsByEventId(event.id);
            } else {
                this.rows = await this.individualStatsService.getIndividualStats();
            }
            this.applySort();
        } catch (error) {
            this.errorMessage = error instanceof Error ? error.message : 'Failed to load individual stats';
        } finally {
